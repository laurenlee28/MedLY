...
const installRequirements = () => {
  return new Promise((resolve, reject) => {
    ...
    const pythonPath = app.isPackaged
      ? path.join(process.resourcesPath, 'medly', 'Scripts', 'python.exe')
      : path.join(app.getAppPath(), 'medly', 'Scripts', 'python.exe');
    
    // --- Determine requirements.txt path for packaged vs. dev environment ---
    const reqPath = app.isPackaged
      ? path.join(process.resourcesPath, 'App', 'requirements.txt')
      : path.join(app.getAppPath(), 'App', 'requirements.txt');
...
const startPythonServer = async () => {
  try {
    ...
    const script = app.isPackaged
      ...
      : path.join(app.getAppPath(), 'App', 'main.py');
